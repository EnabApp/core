<template>
  ...
  <div @click.stop="appManager.setFocus(app.id)" :style="[
    ...
    app.maximized || isMobile
      ? 'w-full h-minus-bottombar'
      : 'border-rounded-[10px]',
  ]" ref="rez">
    <!-- Window Container -->
    <div ref="windowRef" flex="~" place="items-center" justify="center" :class="{
      'w-full h-minus-bottombar': app.maximized || isMobile,
    }">
      <div flex="~ col" :class="[
        app.maximized || isMobile
          ? 'w-full h-minus-bottombar'
          : '' + ' border-rounded-[10px] border-1 border-secondary',
      ]" :style="[
        app.maximized || isMobile ? '' : `width:${w}px;height:${h}px;`,
        `min-width:${app.minWidth}px;min-height:${
          app.minHeight + headerHeight
        }px;`,
      ]" class="relative">
        <div v-if="!(app.maximized || isMobile)">
          <div @mousedown="changeDrag('tl')" position="absolute" cursor="nwse-resize" z="10" left="-1" top="-1" w="20px"
            h="20px"></div>
          <div @mousedown="changeDrag('tr')" position="absolute" cursor="nesw-resize" z="10" right="-1" top="-1"
            w="20px" h="20px"></div>
          <div @mousedown="changeDrag('bl')" position="absolute" cursor="nesw-resize" z="10"
            :style="`left:-12px; top:${content.height + headerHeight - 12}px`" w="20px" h="20px"></div>
          <div @mousedown="changeDrag('br')" position="absolute" cursor="nwse-resize" z="10" :style="`left:${content.width - 12}px; top:${
            content.height + headerHeight - 12
          }px`" w="20px" h="20px"></div>
        </div>
        ...
        <!-- Content -->
        <UiDesktopWindowContent :isMobile="isMobile" :app="app" @size="contentSize($event)">
          <slot />
...
const contentSize = (size) => {
  ...
  if (
    (originalContent.width == 0 || originalContent.height == 0) &&
    !props.app.maximized
  ) {
