<template>
  <div v-if="support.isConnected" flex="~ gap-4 grow" overflow-y="auto" h="100px" w="full">
    ...
    <SupportAssistantMiniState ref="miniState" @closeMiniState="support.miniProfileState = false"
      v-if="support.miniProfileState && (twoXs || xs || sm || md)" :BreakpointWindow="BreakpointWindow" />
  ...
  <div v-else flex="~ gap-4 grow" overflow-y="auto" h="100px" w="full">
    <div flex="~ col " :class="{
      ...
      <SupportAssistantConversationSkeleton v-for="item in 5" :key="'conversation-skeleton-id-' + item"
        :BreakpointWindow="BreakpointWindow" />
    </div>
  </div>
